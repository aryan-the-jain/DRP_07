...
class FacilitatorMessageQueries(private val messagesTable: TableQuery[FacilitatorMessagesTable]) {
  ...
  def selectPrivateMessages(groupId: Int, participantId: Int): Query[
    ...
    val messages = for
      m <- messagesTable if m.groupId === groupId && (m.fromId === participantId || m.toId === participantId)
    yield (m.fromId, m.toId, m.body, m.createdAt)
    messages.sortBy(m => m._4.asc)
  }

  def insertNewMessage(message: FacilitatorMessage): FixedSqlAction[Int, slick.dbio.NoStream, slick.dbio.Effect.Write] = {
